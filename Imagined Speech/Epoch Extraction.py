...
srate = 500  # Hz
epoch_duration = 1.5  # seconds (actual imagination period)
...
for file_name in files:
    ...
    with h5py.File(file_path, 'r') as h5_file:
        ...
        for i, (start_index, label) in enumerate(zip(event_samples, event_classes)):
            ...
            # Extracting only the first 1.5 seconds (750 samples) from the start index
...
# Save path for the extracted epochs
save_path = r'B:\Education\CNN\EEG\Imagined Speech\Processing\Extracted_Epochs.h5'

# Save the extracted epochs and labels to a .h5 file
with h5py.File(save_path, 'w') as h5_file:
    h5_file.create_dataset('epochs', data=all_epochs)
    h5_file.create_dataset('labels', data=np.array(all_labels, dtype='S50'))
...
print(f"Epochs and labels successfully saved to {save_path}.")
