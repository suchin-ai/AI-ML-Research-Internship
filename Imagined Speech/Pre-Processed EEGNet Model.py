...
with h5py.File(r"B:\Education\CNN\EEG\Imagined Speech\Processing\Preprocessed_Epochs.h5", 'r') as f:
...
plt.bar("Pre-Processed-EEGNet (1D)", mean_acc, yerr=std_acc, capsize=10, color="royalblue")
plt.axhline(0.1, color='red', linestyle='--', label='Chance Level')
plt.title(f"Pre-Processed-EEGNet (1D) LORO Accuracy\nMean = {mean_acc:.4f}")
plt.ylabel("Accuracy")
plt.ylim(0, 0.3)
plt.legend()
plt.tight_layout()
