...
class EEGNetDenoiser(nn.Module):
    def __init__(self, in_chans=1, samples=512, dropout_rate=0.25):
        super(EEGNetDenoiser, self).__init__()
        
        self.firstconv = nn.Sequential(
            nn.Conv1d(in_chans, 8, kernel_size=64, stride=1, padding=32, bias=False),
            nn.BatchNorm1d(8)
        )

        self.depthwiseConv = nn.Sequential(
            nn.Conv1d(8, 16, kernel_size=1, groups=8, bias=False),
            nn.BatchNorm1d(16),
            nn.ELU(),
        ...
        self.separableConv = nn.Sequential(
            nn.Conv1d(16, 16, kernel_size=16, padding=8, groups=16, bias=False),
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(16, 16, kernel_size=8, stride=8),
            nn.ELU(),
            nn.ConvTranspose1d(16, 8, kernel_size=4, stride=4),
            nn.ELU(),
...
for noise_type in noise_types:
    ...
    for snr in snr_values:
        ...
        model = EEGNetDenoiser().to(device)
